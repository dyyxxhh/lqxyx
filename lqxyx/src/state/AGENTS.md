...
| 改存档 key | `saveState.ts` | 主存档 `ying-zhong-jiu.checkpoint-save.v1`；JSON 导出包同时包含 replay buffer |
...
## CONVENTIONS
- **存档 key**: 主存档 `ying-zhong-jiu.checkpoint-save.v1`，`schemaVersion = 1`；JSON 导出包使用 `kind: 'ying-zhong-jiu.save-json.v1'`，包含完整 `SaveState` 与 replay buffer。
...
- **JSON 存档**: `exportSaveJson()` / `importSaveJson()` 通过验证后的 JSON 包跨浏览器迁移完整状态；不会用 checkpoint 码重建缩水进度。
