// src/forgottenSanity/ui/SettlementScreen.ts
// 被遗忘的理智撤离/死亡结算：evacuated（达标入仓库+更新 best）/ refused（不达标拒绝）/ dead（全丢）。
...
export class SettlementScreen {
  ...
  private handleEvacuated(inventory: Inventory, total: number): SettlementOutcome {
    // 1. 并入仓库
    const stash: ForgottenSanityStashState = loadStashState().state;
