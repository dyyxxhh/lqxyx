...
SOURCES = ["秦浩瑞.png", "但宇轩.png"]
ACTIONS = ["站立向右", "横躺无血", "横躺有血", "头部部件", "身体部件"]
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path.startswith("/api/images"):
            ...
            for name in SOURCES:
                p=MATERIAL_DIR/name
                if p.exists():
                    files.append({"name":name,"character":Path(name).stem,"url":f"/materials/{name}"})
            self.send_json({"ok":True,"files":files,"actions":ACTIONS})
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        if self.path.startswith("/api/save"):
            ...
            try:
                ...
                self.send_json({"ok":False,"error":str(exc)},status=400); return
            self.send_json({"ok":True,"saved":saved}); return
        self.send_error(404)
...
def main()->None:
    ...
    parser.add_argument("--port",type=int,default=16624)
    args=parser.parse_args()
    os.chdir(ROOT)
    httpd=ThreadingHTTPServer((args.host,args.port),Handler)
    print(f"Two-point cropper: http://127.0.0.1:{args.port}")
