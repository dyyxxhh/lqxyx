// src/forgottenSanity/meta/LoadoutManager.ts
// 被遗忘的理智起配数据层：1 武器 + 3 消耗品槽（武备 +1 最多 +3→6）、空手 unarmed、纯函数消费仓库。
// 纯 TS，无 Phaser import。spec §8.3，plan 6 Task 4。
import type { ForgottenSanityUpgradesState, ForgottenSanityStashState } from '../state/forgottenSanityState';
...
export function getConsumableSlotCount(
  tiers: Readonly<Record<keyof ForgottenSanityUpgradesState['tiers'], number>>,
): number {
