...
import { ForgottenSanityHUD, HUD_BASE_DEPTH, HUD_TEXT_DEPTH, HUD_OVERLAY_DEPTH } from '../forgottenSanity/ui/ForgottenSanityHUD';
import type { HudSnapshot } from '../forgottenSanity/ui/ForgottenSanityHUD';
...
describe('ForgottenSanityHUD depth constants (spec §9.1 / §11.5)', () => {
...
describe('ForgottenSanityHUD lifecycle', () => {
  it('create renders HP bar, stamina bar, weapon name, ult CD, sanity, fragment count, timer, consumable slots, sanity ratio', () => {
    ...
    const hud = new ForgottenSanityHUD(env.scene);
    hud.create();
    hud.update(snapshot());
