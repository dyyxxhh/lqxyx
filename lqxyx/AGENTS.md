...
| 改角色初始位置/存档码 | `src/state/saveState.ts` | 默认 4F 走廊中心 `560,920,down`，蓝边；四位自包含进度码 |
...
- **存档**: 主存档 `localStorage` key `ying-zhong-jiu.checkpoint-save.v1`，schemaVersion = 1；四位进度码是自包含 checkpoint 编码，不依赖本机映射表。
