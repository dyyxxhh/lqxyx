...
import type { ForgottenSanityUpgradesState, ForgottenSanityStashState } from '../forgottenSanity/state/forgottenSanityState';
import type { Loadout } from '../forgottenSanity/meta/LoadoutManager';

function tiers(over: Partial<Record<keyof ForgottenSanityUpgradesState['tiers'], number>> = {}): ForgottenSanityUpgradesState['tiers'] {
  return { physique: 0, swift: 0, pickup: 0, sharp: 0, lucky: 0, armory: 0, ...over };
}

function stash(sanity: number, items: { itemId: string; quantity: number }[] = []): ForgottenSanityStashState {
