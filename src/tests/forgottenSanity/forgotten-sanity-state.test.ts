...
import {
  FORGOTTEN_SANITY_BEST_STORAGE_KEY,
...
describe('forgottenSanityState 常量与默认态', () => {
  beforeEach(() => localStorage.clear());

  it('四个 localStorage key 与 schema 版本', () => {
    expect(FORGOTTEN_SANITY_STASH_STORAGE_KEY).toBe('ying-zhong-jiu.forgotten-sanity.stash.v1');
    expect(FORGOTTEN_SANITY_UPGRADES_STORAGE_KEY).toBe('ying-zhong-jiu.forgotten-sanity.upgrades.v1');
    expect(FORGOTTEN_SANITY_BEST_STORAGE_KEY).toBe('ying-zhong-jiu.forgotten-sanity.best.v1');
    expect(FORGOTTEN_SANITY_PROGRESS_STORAGE_KEY).toBe('ying-zhong-jiu.forgotten-sanity.progress.v1');
    expect(FORGOTTEN_SANITY_SCHEMA_VERSION).toBe(1);
  });
...
describe('forgottenSanityState 读写往返', () => {
  beforeEach(() => localStorage.clear());
  ...
  it('损坏 JSON 返回 invalid + corrupt-json', () => {
    localStorage.setItem(FORGOTTEN_SANITY_STASH_STORAGE_KEY, '{not-json');
    const loaded = loadStashState();
