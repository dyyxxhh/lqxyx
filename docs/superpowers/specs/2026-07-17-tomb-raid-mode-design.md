...
- **冲撞**：间隔 3s, 蓄力 1.0s, 持续 0.7s, 速度 320, 伤害 50 + 击退
...
progress: 0..1, rate = 1/2500 per ms
...
### §8.2 商城（Shop）
- 卖价 = 碎片面值 ×1（1:1）
- 买价 = `Math.ceil(lootItem.sanityValue × 1.35)`
- 可买：消耗品 + 武器；可卖：任意
