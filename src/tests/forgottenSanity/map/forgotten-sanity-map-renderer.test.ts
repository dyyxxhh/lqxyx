...
import type { ForgottenSanityMapManifest, ForgottenSanityRect } from '../../../forgottenSanity/map/forgottenSanityMapState';
import { ForgottenSanityMapRenderer } from '../../../forgottenSanity/map/ForgottenSanityMapRenderer';
...
interface TrackedObj {
  ...
  // graphics ops
  fillStyle?: number;
  fillRect?: ForgottenSanityRect;
  lineStyle?: { width: number; color: number };
  strokeRect?: ForgottenSanityRect;
  // rectangle
  color?: number;
  // text
  text?: string;
...
function makeMiniManifest(overrides: Partial<ForgottenSanityMapManifest> = {}): ForgottenSanityMapManifest {
  const base: ForgottenSanityMapManifest = {
...
describe('ForgottenSanityMapRenderer', () => {
  it('render 创建 floor tile image（depth=0）', () => {
    const { scene, tracked } = createMockScene();
    const renderer = new ForgottenSanityMapRenderer(scene as never);
