...
| 改存档 key | `saveState.ts` | 主存档 `ying-zhong-jiu.checkpoint-save.v1`；本机存档码 codebook `ying-zhong-jiu.save-codebook.v1` |
...
## CONVENTIONS
- **存档 key**: 主存档 `ying-zhong-jiu.checkpoint-save.v1`，`schemaVersion = 1`；本机四位存档码使用 `ying-zhong-jiu.save-codebook.v1` 映射到序列化存档。
...
- **持久化**: `saveSaveState()` 写 `localStorage`；`loadSaveState()` 读并验证；`clearSaveState()` 删除主存档和 replay buffer，保留本机存档码 codebook。
- **本机存档码**: `exportSaveCode()` / `importSaveCode()` 只接受四位数字，依赖同浏览器 `save-codebook`，不是跨设备自包含存档。
